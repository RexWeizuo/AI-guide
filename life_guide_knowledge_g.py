...
class CognitiveMemory:
    def __init__(self):
        ...
        self.knowledge_graph = []    # 知识节点关系图

    def update_memory(self, new_insight):
        # 认知更新算法
        if self._is_conflict(existing_knowledge):
            self.trigger_cognitive_dissonance_resolution()
        self.graph.add_node(new_insight)

    def retrieve_context(self, query):
        # 相关性+时效性加权检索
        return self.vector_db.similarity_search(
            query,
            filter=lambda x: x.metadata["last_used"] > time.time()-30*86400
        )
